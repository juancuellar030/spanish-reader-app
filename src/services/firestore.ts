...
export function normalizeStudentDoc(
  id: string,
  data: Record<string, unknown>,
): Student {
  ...
  return {
    ...
    collectibles: Array.isArray(data.collectibles)
      ? (data.collectibles as string[])
      : [],
    lastAssignedWeek:
      typeof data.lastAssignedWeek === "string"
        ? data.lastAssignedWeek
        : undefined,
    assignmentDeadline:
      typeof data.assignmentDeadline === "number"
        ? data.assignmentDeadline
        : undefined,
  };
}
...
export const getStudent = async (
  ...
  // #region agent log
  debugLog(
    "firestore.ts:getStudent:entry",
    "getStudent called",
    { studentId, ...firestoreContext() },
    "H2",
  );
  // #endregion
  try {
    ...
    // #region agent log
    debugLog(
      "firestore.ts:getStudent:success",
      "getStudent completed",
      { studentId, exists: docSnap.exists(), ...firestoreContext() },
      "H2",
    );
    // #endregion
    if (docSnap.exists()) {
      return normalizeStudentDoc(
        docSnap.id,
    ...
    // #region agent log
    debugLog(
      "firestore.ts:getStudent:error",
      "getStudent failed",
      { studentId, code: e?.code, message: e?.message, ...firestoreContext() },
      "H1,H2,H4,H5",
    );
    // #endregion
    throw err;
...
export const getAllStudents = async (): Promise<Student[]> => {
  // #region agent log
  debugLog(
    "firestore.ts:getAllStudents:entry",
    "getAllStudents called",
    { collection: "students", ...firestoreContext() },
    "H1,H2,H5",
  );
  // #endregion
  try {
    ...
    // #region agent log
    debugLog(
      "firestore.ts:getAllStudents:success",
      "getAllStudents completed",
      { count: snapshot.size, ...firestoreContext() },
      "H1,H2,H5",
    );
    // #endregion
    ...
    const newStudents = students.filter(
      (s) => s.id === "student-94" || s.id === "student-95",
    );
    // #region agent log
    debugLog(
    ...
    // #region agent log
    debugLog(
      "firestore.ts:getAllStudents:error",
      "getAllStudents failed",
      { code: e?.code, message: e?.message, ...firestoreContext() },
      "H1,H2,H4,H5",
    );
    // #endregion
    throw err;
...
export const awardCollectible = async (
  studentId: string,
  storyId: string,
): Promise<void> => {
...
export const getStudentProgress = async (
  ...
  return snapshot.docs.map(
    (doc) => ({ storyId: doc.id, ...doc.data() }) as Progress,
  );
};
...
export const getWeeklyAssignments = async (weekId: string) => {
  // #region agent log
  debugLog(
    "firestore.ts:getWeeklyAssignments:entry",
    "getWeeklyAssignments called",
    { weekId, collection: "assignments", ...firestoreContext() },
    "H1,H3,H5",
  );
  // #endregion
  try {
    ...
    // #region agent log
    debugLog(
      "firestore.ts:getWeeklyAssignments:success",
      "getWeeklyAssignments completed",
      { weekId, exists: docSnap.exists(), ...firestoreContext() },
      "H1,H3,H5",
    );
    // #endregion
    if (docSnap.exists()) {
    ...
    // #region agent log
    debugLog(
      "firestore.ts:getWeeklyAssignments:error",
      "getWeeklyAssignments failed",
      { weekId, code: e?.code, message: e?.message, ...firestoreContext() },
      "H1,H3,H4,H5",
    );
    // #endregion
    throw err;
...
export const saveWeeklyAssignments = async (
  weekId: string,
  ...
  const allStudents = studentsSnapshot.docs.map(
  ...
  for (const student of allStudents) {
    ...
    // 1. Update the student's assignedStories array on their main doc, also stamp the week and deadline
