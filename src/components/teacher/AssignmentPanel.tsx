...
export const AssignmentPanel = () => {
  ...
  const [deadlineValue, setDeadlineValue] = useState<number>(7);
  const [deadlineUnit, setDeadlineUnit] = useState<"days" | "weeks">("days");
  ...
  const availableStories: Story[] = (storiesData as Story[]).filter(
  ...
  const handleToggleStory = (storyId: string) => {
    setAssignments((prev) => {
      ...
      if (currentGradeAssignments.includes(storyId)) {
        ...
        return {
          ...prev,
          [selectedGrade]: currentGradeAssignments.filter(
            (id) => id !== storyId,
          ),
        };
      } else {
        ...
        if (currentGradeAssignments.length >= 3) {
          alert(
            "Solo puedes asignar un máximo de 3 historias por semana para este grado.",
          );
          return prev;
        }
        return {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const multiplier = deadlineUnit === "days" ? 86400000 : 604800000;
      const deadline = Date.now() + deadlineValue * multiplier;
      await saveWeeklyAssignments(weekId, assignments, deadline);
      setShowSuccess(true);
        ...
        <div className="flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-xl border border-gray-200">
            ...
            <input
              ...
              className="w-16 px-2 py-1 text-center border border-gray-300 rounded-md focus:outline-none focus:border-brand"
            />
            <select
              value={deadlineUnit}
              onChange={(e) =>
                setDeadlineUnit(e.target.value as "days" | "weeks")
              }
              className="px-2 py-1 border border-gray-300 rounded-md focus:outline-none focus:border-brand bg-white"
            >
        ...
        <div className="md:w-48 border-b md:border-b-0 md:border-r border-gray-100 p-4 flex md:flex-col gap-2 overflow-x-auto">
          {[1, 2, 3, 4, 5].map((grade) => (
            <button
              ...
              className={`px-4 py-3 rounded-xl font-semibold text-left whitespace-nowrap transition-colors ${
                selectedGrade === grade
        ...
        <div className="flex-1 p-6 bg-soft-gray/30">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {availableStories.map((story) => {
              const isAssigned = (assignments[selectedGrade] || []).includes(
                story.id,
              );
              ...
              return (
                <motion.div
                  key={story.id}
                  whileHover={{ y: -4 }}
                  className={`relative bg-white rounded-2xl p-4 cursor-pointer transition-all border-2 ${
                    isAssigned
                      ? "border-brand shadow-md"
                      : "border-transparent shadow-sm hover:border-brand/30"
                  }`}
                ...
                >
                  <div className="aspect-[4/3] rounded-xl overflow-hidden mb-4 relative">
                    ...
                    {isAssigned && (
                      <div className="absolute inset-0 bg-brand/20 flex items-center justify-center backdrop-blur-[1px]">
                        <div
                          className="bg-brand text-white p-2 rounded-full shadow-lg"
                          style={{ backgroundColor: BRAND_PRIMARY }}
                        >
                          <Check size={24} />
                        </div>
                  ...
                  <p className="text-xs text-gray-400 mt-2 font-medium">
                    {story.type === "flipbook" ? "📖 Flipbook" : "🎵 Karaoke"} •{" "}
                    {story.wordCount} p.
                  </p>
                </motion.div>
              );
