...
export const StudentLogin = ({ onLogin }: StudentLoginProps) => {
  ...
  useEffect(() => {
    const probe = () => {
      ...
      const temasEl = document.querySelector(
        "[data-debug-temas]",
      ) as HTMLElement | null;
      ...
      const payload = {
        ...
        data: {
          gradeCircle_bgColor: gradeEl
            ? window.getComputedStyle(gradeEl).backgroundColor
            : "no-el",
          gradeCircle_classes: gradeEl?.className ?? "no-el",
          temas_bgColor: temasEl
            ? window.getComputedStyle(temasEl).backgroundColor
            : "no-el",
          temas_classes: temasEl?.className ?? "no-el",
          btn_bgColor: btnEl
            ? window.getComputedStyle(btnEl).backgroundColor
            : "no-el",
        },
        timestamp: Date.now(),
      ...
      fetch(
        "http://127.0.0.1:7623/ingest/d6025ec4-1902-461d-ae6d-4b4976ec2ce2",
        {
          method: "POST",
          headers: {
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    if (
      selectedStudent &&
      password.toUpperCase() === selectedStudent.password
    ) {
      try {
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-4 overflow-hidden">
          ...
          <div className="relative flex flex-col items-end">
            <div className="relative">
              <motion.button
                ...
                className={`relative flex items-center gap-2 px-4 py-2 rounded-full shadow-lg text-white font-semibold font-poppins transition-colors ${BRAND_FILL_CLASS} ${
              ...
              >
                ...
                <Palette
                  size={20}
                  className={!hasEngagedWithThemes ? "animate-pulse" : ""}
                />
                <span>Temas</span>
                <ChevronDown
                  size={16}
                  ...
                  >
                    {[
                      ...
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors ${theme.bg} ${
                          currentTheme === theme.id
                            ? "bg-gray-100/50 font-bold"
                            : "font-medium"
              ...
              {!hasEngagedWithThemes && !isThemeMenuOpen && (
                ...
                >
                  ...
                  <p className="text-xs text-gray-600 mt-1">
                    Toca <span className="font-bold">Temas</span> antes de
                    entrar
                  </p>
                </motion.div>
              )}
      ...
      >
        {/* Greeting */}
        <motion.h1
          className={`text-5xl md:text-6xl font-bold text-center mb-8 transition-all duration-500 ${
              ...
              <div className="bg-white rounded-2xl shadow-lg max-h-96 overflow-y-auto overflow-x-hidden custom-scrollbar">
                {isLoading ? (
                  <div className="p-8 text-center text-charcoal flex flex-col items-center justify-center">
                    <Loader2
                      className={`animate-spin mb-4 ${BRAND_TEXT_CLASS}`}
          ...
          >
            ...
            <div className="text-center mt-6">
              <h2 className="text-2xl font-bold text-charcoal mb-2">
                ¡Hola, {selectedStudent.name.split(" ")[0]}!
              </h2>
              ...
              <form onSubmit={handlePasswordSubmit}>
                <div className="mb-4 relative flex items-center">
                  <input
                    ...
                    placeholder="······"
                    className={`w-full text-center tracking-widest text-2xl h-14 rounded-xl border-2 focus:outline-none transition-colors uppercase pr-12 ${
                      error
      ...
      <motion.button
        ...
        onClick={() =>
          window.dispatchEvent(new CustomEvent("teacher-login-request"))
        }
